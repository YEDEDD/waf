        ...
        if IP_WHITE_RULE ~= nil then
            for _,rule in pairs(IP_WHITE_RULE) do
                if rule ~= "" and rulematch(WHITE_IP,rule,"jo") then
                    ...
                    log_record_normal('White_IP',ngx.var_request_uri,"_","_")
                    return true
                end
            end
        end
        ...
        if IP_BLACK_RULE ~= nil then
            for _,rule in pairs(IP_BLACK_RULE) do
                if rule ~= "" and rulematch(BLACK_IP,rule,"jo") then
                    ...
                    log_record_blocked('BlackList_IP',ngx.var_request_uri,"_","_")
        ...
        if USER_COOKIE ~= nil then
            for _,rule in pairs(COOKIE_RULES) do
                if rule ~="" and rulematch(USER_COOKIE,rule,"jo") then
                    log_record_blocked('Deny_Cookie',ngx.var.request_uri,"-",rule)
                    if config_waf_enable == "on" then
                        waf_output()
                        return true
                    end
...
--deny url Domain Name
function domain_attack_check()
    if config_domain_check == "on" then
        local DOMAIN_RULES = get_rule('domain.rule')
        local REQ_DOMAIN = ngx.var.host

        for _, rule in pairs(DOMAIN_RULES) do
            if rule ~= "" and rulematch(REQ_DOMAIN, rule, "jo") then
                log_record_blocked('Deny_DOMAIN', REQ_DOMAIN, "-", rule)
                if config_waf_enable == "on" then
                    waf_output()
                    return true
                end
...
function url_attack_check()
    if config_url_check == "on" then
        ...
        local REQ_PORT = ngx.var.server_port
        local REQUEST_WITH_PORT = ":" .. REQ_PORT .. REQ_URI -- 将端口号与路径组合

        for _, rule in pairs(URL_RULES) do
            if rule ~= "" and rulematch(REQUEST_WITH_PORT, rule, "jo") then
                log_record_blocked('Deny_URL', REQUEST_WITH_PORT, "-", rule)
                if config_waf_enable == "on" then
                    waf_output()
                    return true
                end
        ...
        for _,rule in pairs(ARGS_RULES) do
            ...
            for key, val in pairs(REQ_ARGS) do
                ...
                if ARGS_DATA and type(ARGS_DATA) ~= "boolean" and rule ~="" and rulematch(unescape(ARGS_DATA),rule,"jo") then
                    log_record_blocked('Deny_URL_Args',ngx.var.request_uri,"-",rule)
                    if config_waf_enable == "on" then
                        waf_output()
                        return true
                    end
